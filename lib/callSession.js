...
class CallSession {
  ...
  async _processUtterance() {
    ...
    try {
      ...
      const wav = createWavBuffer(pcm, SAMPLE_RATE);
      this.log(`STT: ${raw.length} mulaw bytes → ${wav.length} wav bytes`);

      const { text, detectedLang } = await transcribe(wav);
      ...
      if (this.firstTurn) {
        ...
        if (this.lang !== 'en') {
          ...
          const confirmLocal = await translate(confirmEN, 'English', langName);
      ...
      if (this.lang && this.lang !== 'en') {
        const langName = getLanguageName(this.lang);
        queryEN = await translate(text, langName, 'English');
      ...
      if (this.lang && this.lang !== 'en') {
        const langName = getLanguageName(this.lang);
        answerLocal = await translate(answerEN, 'English', langName);
  ...
  async speak(text) {
    ...
    try {
      ...
      const pcm24k = await synthesize(text, this.voice);
      ...
      // Convert 24→8 kHz and send chunks
      const mulaw = ttsToTwilio(pcm24k);
  ...
  async _recoverGracefully() {
    try {
      ...
      if (this.lang && this.lang !== 'en') {
        msg = await translate(msg, 'English', getLanguageName(this.lang));
      }
